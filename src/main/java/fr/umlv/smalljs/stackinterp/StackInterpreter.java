    ...
    for (;;) {
      switch (instrs[pc++]) {
        ...
        case Instructions.LOOKUP -> {
          ...
          // lookup the name
          var value = (JSObject) globalEnv.lookupOrDefault(name, null);
          // if it does not exist throw a failure
          if (value == null) {
            throw new Failure("IDK");
          }
          // otherwise push as any anyValue
          var ret = encodeAnyValue(value, dict);
          push(stack, sp++, ret);
        ...
        case Instructions.FUNCALL -> {
          ...
          /*
           * { // DEBUG
           * var receiver = decodeAnyValue(stack[baseArg + RECEIVER_BASE_ARG_OFFSET],
           * dict, heap);
           * var args = new Object[argumentCount];
           * for (var i = 0; i < argumentCount; i++) {
           * args[i] = decodeAnyValue(stack[baseArg + i], dict, heap);
           * }
           * System.err.println("funcall " + newFunction.getName() + " with " + receiver +
          ...
          if (code.parameterCount() != argumentCount + 1/* this */) {
            throw new Failure("wrong number of arguments for " + newFunction.name() +
                "expected " + (code.parameterCount() - 1) + " but was " + argumentCount);
          }
